...
public class OllehMetaController  extends CommonController {
    ...
    {
    	logger.info("[저장시 메타키워드 누락 테스트] - OllehMetaController.apis /v1/apis");
        String apiInfo   = request.getParameter("apiUrl");
        String apiParam  = request.getParameter("apiParam");
    	//logger.info("[저장시 메타키워드 누락 테스트] - apiInfo - " + apiInfo);
    	//logger.info("[저장시 메타키워드 누락 테스트] - apiParam - " + apiParam);
    	...
    	logger.info("[저장시 메타키워드 누락 테스트] - OllehMetaController.apis httpClientUtil.requestOllehMeta(apiInfoMap, apiParamMap) 동작");
        String response = httpClientUtil.requestOllehMeta(apiInfoMap, apiParamMap);
    	logger.info("[저장시 메타키워드 누락 테스트] - OllehMetaController.apis response - " + response);
        return response;
    }
